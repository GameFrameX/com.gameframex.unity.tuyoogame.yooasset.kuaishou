...
`com.gameframex.unity.tuyoogame.yooasset.kuaishou` 为 Unity WebGL 的**快手小游戏**平台提供 YooAsset `IFileSystem` 适配，使 GameFrameX 游戏能够通过快手小游戏运行时 SDK 加载并缓存 AssetBundle 资源。

## 功能特性

- 面向快手小游戏运行时的专用 `IFileSystem` 实现。
- 与快手 SDK 的 AssetBundle 下载与本地缓存深度集成。
- 通过 YooAsset WebPlayMode 流程对接：包版本请求、清单加载、资源包下载/加载。
- 可插拔的远端服务（CDN、自定义源、解密密钥）与内置资源解密钩子。
...
3. 在 Unity 的 **Package Manager** 中使用 **Git URL** 的方式添加库，地址为：`https://github.com/gameframex/com.gameframex.unity.tuyoogame.yooasset.kuaishou.git`

4. 直接下载仓库放置到 Unity 项目的 `Packages` 目录下，会自动加载识别。

### 使用示例

将快手文件系统接入 YooAsset 的 WebPlayMode 初始化：
...
查看 [Releases](https://github.com/gameframex/com.gameframex.unity.tuyoogame.yooasset.kuaishou/releases) 了解更新日志。
